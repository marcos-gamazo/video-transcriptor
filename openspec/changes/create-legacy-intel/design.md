...
* Macs Intel con macOS 12 Monterey como sistema máximo soportado (p. ej. MacBook Air 2015, 8 GB RAM).
* Equipos sin Neural Engine de Apple.

En macOS 12 para Intel, la única API de transcripción de Apple disponible es `SFSpeechRecognizer`, que en dispositivos sin Neural Engine requiere conexión con los servidores de Apple. Esto viola los requisitos de privacidad y offline de la aplicación.
...
* macOS 12.0 (Monterey) como deployment target mínimo.
...
* Soporte para macOS anterior a 12.0 (Monterey).
...
La biblioteca Vosk se compilará como un `libvosk.a` estático x86_64 para macOS 12, optimizado con `-O2` y enlazado con Accelerate (si aplica) o Accelerate framework.
